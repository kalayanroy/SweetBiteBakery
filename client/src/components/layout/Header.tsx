          ...
          <div className="flex justify-between items-center h-16 md:h-20">
            ...
            <div className="flex items-center">
              <Link href="/">
                <div className="flex items-center cursor-pointer">
                  <span className="text-primary text-xl sm:text-2xl md:text-3xl font-heading font-bold tracking-tight">SweetBite</span>
                  <span className="ml-1 text-accent/80 text-[10px] sm:text-xs uppercase tracking-widest font-semibold hidden xs:inline-block">Bakery</span>
                </div>
              </Link>
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-6">
              ...
              <button
                ...
                className="flex items-center text-text-dark hover:text-primary transition-colors cursor-pointer"
            ...
            <nav className="hidden md:flex lg:hidden items-center">
              <div className="flex items-center space-x-4">
                <Link href="/">
                  <div className={`flex flex-col items-center text-text-dark hover:text-primary transition-colors duration-300 ${location === '/' ? 'text-primary' : ''}`}>
                    <HomeIcon className="h-5 w-5" />
                    <span className="text-xs mt-1">Home</span>
                  </div>
                </Link>
                <Link href="/products">
                  <div className={`flex flex-col items-center text-text-dark hover:text-primary transition-colors duration-300 ${location === '/products' ? 'text-primary' : ''}`}>
                    <ShoppingBag className="h-5 w-5" />
        ...
        <div className="fixed inset-0 z-[60] bg-white flex flex-col md:hidden">
          ...
          <div className="flex items-center justify-between px-4 h-16 border-b border-[#e3d9c8]">
            <Link href="/" onClick={() => setMobileMenuOpen(false)}>
              <div className="flex items-center cursor-pointer">
                <span className="text-primary text-xl font-heading font-bold tracking-tight">SweetBite</span>
                <span className="ml-1 text-accent/80 text-[10px] uppercase tracking-widest font-semibold">Bakery</span>
              </div>
            </Link>
            <button 
              className="text-primary p-2" 
          ...
          <nav className="flex-1 overflow-y-auto py-8 px-6">
            <ul className="space-y-6">
              <li>
                <Link href="/" onClick={() => setMobileMenuOpen(false)}>
                  <div className={`text-lg font-medium ${location === '/' ? 'text-primary' : 'text-text-dark'}`}>
                    Home
                  </div>
                </Link>
              </li>
              <li>
                <Link href="/products" onClick={() => setMobileMenuOpen(false)}>
                  <div className={`text-lg font-medium ${location === '/products' ? 'text-primary' : 'text-text-dark'}`}>
                    Products
                  </div>
                </Link>
              </li>
